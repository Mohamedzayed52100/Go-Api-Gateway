...
'''
    ...
    environment {
        IMAGE_REGISTRY_PROD="348277991470.dkr.ecr.eu-west-2.amazonaws.com/goplace-gateway-prod"
        IMAGE_REGISTRY_DEV="348277991470.dkr.ecr.eu-west-2.amazonaws.com/goplace-gateway-dev"
        IMAGE_REGISTRY_STAGING="348277991470.dkr.ecr.eu-west-2.amazonaws.com/goplace-gateway-staging"
        K8S_MANIFESTS_BRANCH_DEV="goplace-gateway-dev"
        K8S_MANIFESTS_BRANCH_PROD="goplace-gateway-prod"
        K8S_MANIFESTS_BRANCH_STAGING="goplace-gateway-staging"
        ...
        container("docker-dind") {
            script {
                withCredentials([string(credentialsId: 'GitHubAccessToken', variable: 'GITHUB_ACCESS_TOKEN')]) {
                    sh """
                        docker run --rm -v \$(pwd):/local goplace/openapi-generator:latest -- -r /local -f ./openapi/specs/openapi.yaml -o ./openapi
